...
impl GigaAMModel {
    ...
    /// Публичный доступ к сырым CTC log-вероятностям `[1, T', V]` без декодирования —
    /// для внешнего декодера (greedy + contextual biasing) в golos-sidecar. Добавлено
    /// для golos: апстрим отдаёт только `.text` из `transcribe`/`transcribe_raw`.
    /// Слишком короткий вход → массив `[1, 0, 0]`.
    pub fn infer_log_probs(
        &mut self,
        samples: &[f32],
    ) -> Result<ndarray::Array3<f32>, TranscribeError> {
        if samples.len() < self.mel_config.n_fft {
            return Ok(ndarray::Array3::zeros((1, 0, 0)));
        }
    ...
    /// Вокабуляр модели: индекс токена → строка (SentencePiece). Blank — `blank_idx`.
    pub fn vocab(&self) -> &[String] {
        &self.vocab
    }

    /// Индекс blank-токена CTC.
    pub fn blank_idx(&self) -> i64 {
        self.blank_idx
    }
